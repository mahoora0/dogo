...
class AnimalReportSchemaMigratorTest {
	...
	@Test
	void migrateReplacesOutdatedReportTypeCheckConstraint() throws Exception {
		DataSource dataSource = dataSource();
		JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
		jdbcTemplate.execute("""
				CREATE TABLE ANIMAL_REPORT (
				  ...
				  REGDATE DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
				  MODDATE DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
				  CONSTRAINT CK_ANIMAL_REPORT_TYPE CHECK (REPORT_TYPE IN ('MISSING', 'SIGHTING', 'PROTECTING'))
				)
				""");
		...
		String checkClause = jdbcTemplate.queryForObject(
				"SELECT CHECK_CLAUSE FROM INFORMATION_SCHEMA.CHECK_CONSTRAINTS WHERE CONSTRAINT_NAME = 'CK_ANIMAL_REPORT_TYPE'",
				String.class);
		assertThat(checkClause).contains("TRANSFERRED").contains("RETURNED");
	}

	private DataSource dataSource() {
		DriverManagerDataSource dataSource = new DriverManagerDataSource();
		dataSource.setDriverClassName("org.h2.Driver");
		dataSource.setUrl("jdbc:h2:mem:animal_report_migrator_" + java.util.UUID.randomUUID()
