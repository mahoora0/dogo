...
class FoundItemControllerTest {
	...
	void listAddsSearchResultAndFiltersToModel() throws Exception {
		...
		when(foundItemService.search(eq("지갑"), eq("ITEM_CATEGORY"), eq("지갑"), eq("강남"), eq("KEEPING"), any(Pageable.class)))
		...
		verify(foundItemService).search(eq("지갑"), eq("ITEM_CATEGORY"), eq("지갑"), eq("강남"), eq("KEEPING"), any(Pageable.class));
	...
	void createRedirectsAnonymousUserToLogin() throws Exception {
		...
		verify(foundItemService, never()).create(any(), any());
	}
