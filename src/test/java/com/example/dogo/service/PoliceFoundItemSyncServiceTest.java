...
class PoliceFoundItemSyncServiceTest {
	...
	void incrementalSyncFetchesDetailAndStopsAfterDuplicatePages() {
		...
		when(client.fetchFoundItems(startDate, endDate, 1, 100)).thenReturn(page(item("F202605110000001", "1")));
		when(client.fetchFoundItems(startDate, endDate, 2, 100)).thenReturn(page(item("F202605100000001", "1")));
		when(client.fetchFoundItems(startDate, endDate, 3, 100)).thenReturn(page(item("F202605090000001", "1")));
		...
		verify(client, never()).fetchFoundItems(startDate, endDate, 4, 100);
	...
	void backfillSyncFetchesDetailUntilEmptyPage() {
		...
		when(client.fetchFoundItems(startDate, endDate, 1, 100)).thenReturn(page(newItem));
		when(client.fetchFoundItems(startDate, endDate, 2, 100)).thenReturn(page());
		when(client.fetchFoundItemDetail("F202605110000001", 1)).thenReturn(Optional.of(detail("F202605110000001", "1")));
		when(foundItemRepository.existsByAtcIdAndFdSn("F202605110000001", 1)).thenReturn(false);
